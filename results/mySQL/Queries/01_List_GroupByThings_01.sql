...
select ptm, count(AC) count, FROM_AA, TO_AA, ft from sprot_ptms2 where ft not like "NOFT" group by PTM, FROM_AA order by count desc
into outfile "/tmp/mySQL/03_List_GroupByThings_sprot_ptms2.csv" FIELDS TERMINATED BY '&' ENCLOSED BY '%'LINES TERMINATED BY '\n';
