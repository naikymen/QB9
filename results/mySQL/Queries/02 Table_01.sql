...
select ptm, count(AC) count, FROM_AA, TO_AA, from_res, to_res, ft from sprot_ptms1 where ft not like "NOFT" and (from_res like '?' or to_res like '?') group by PTM, FROM_AA order by count desc
#into outfile "/tmp/mySQL/01_List_GroupByThings_00.csv" FIELDS TERMINATED BY '&' ENCLOSED BY '%'LINES TERMINATED BY '\n'
;
