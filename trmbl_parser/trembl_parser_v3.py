...
# Abrir el .dat de TREMBL
trembl_file = expanduser("~") + '/QB9_Files/uniprot_trembl.dat'
...
cur.execute("CREATE TABLE IF NOT EXISTS trembl_count_aa (AC_NUM VARCHAR(30), OC_ID VARCHAR(30), LENGTH MEDIUMINT,"
      + table_def
      + ") ENGINE=InnoDB")
con.commit()
...
cur.execute("CREATE TABLE IF NOT EXISTS trembl (" + table_def_2 + ") ENGINE=InnoDB")
con.commit()
...
listaq = []
listar = []
with open(trembl_file) as trembl:  # esto me abre y cierra el archivo al final
    for record in SwissProt.parse(trembl):
        i += 1
        data = empty_data
        listaq = []
        ...
        cur.execute("INSERT INTO trembl_count_aa VALUES ('"
              + record.accessions[0] + "', '"
                ...
                if feature[0] in interes:  # si el titulo del FT interesa, proseguir ¡mejora un poco! =D
                    for tipo in interes:  # iterear los tipos interesantes encontrados en el feature
                        if feature[0] in tipo:  # si el feature evaluado interesante, cargar los datos en data[]
                            ...
                            cur.execute(("INSERT INTO trembl VALUES (%r);"
                                         % sql_insert_values_p + '\n').replace("\"", '').replace('.', ''))
                            con.commit()
